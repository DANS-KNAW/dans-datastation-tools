...
def report_manage_deposit_data(server_url, command_line_parser):
    response = requests.get(server_url + "?" + str(command_line_parser.filter_params), headers=command_line_parser.headers)
    print(response.text)


def notify_func_admin(to_email, subject, message_body, attachment):
...
def main():
    ...
    report_manage_deposit_data(server_url, CommandLineParser(args))
